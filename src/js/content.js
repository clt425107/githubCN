const allData = [
  ...
  [`GitHub Pages is currently disabled. Select a source below to enable GitHub Pages for this repository`, `GitHub Pages目前被禁用。选择下面的源代码，为这个存储库启用GitHub Pages`],
  [`With a GitHub Enterprise account, you can restrict access to your GitHub Pages site by publishing it privately. You can use privately published sites to share your internal documentation or knowledge base with members of your enterprise. You can try GitHub Enterprise risk-free for 30 days`, `有了GitHub企业帐户，你可以通过私下发布来限制访问你的GitHub页面网站。您可以使用私有发布的站点与企业成员共享您的内部文档或知识库。你可以尝试GitHub企业无风险30天`],
  [`Branch`, `分支`],
  ...
  [`Password`, `密码`],
  [`Sign in`, `登录`],
  [`Registration is not currently supported`, `刚来 Github？`],
  ...
  [
    `Build, test, and deploy your code. Make code reviews,  management, and issue triaging work the way you want. Select a workflow to get started.`,
  ...
  [`Clear status`, `清除状态`],
  [`Never`, `从不`],
  [`Your main  isn't protected`, `你的 main 分支不是受保护的`],
  [
    `Protect this  from force pushing or deletion, or require status checks before merging.`,
    `保护这个分支不被强行推送或删减，或要求在合并前进行状态检查 `,
  ],
  [`Learn more`, `学习更多`],
  [` protection rule`, `分支保护规则`],
  [`Protect your most important es`, `保护你最重要的分支`],
  [`Achievements`, `成就`],
  [`Send feedback`, `发送反馈`],
  ...
  [`Pushes`, `推送`],
  [
    `Limit how many es and tags can be updated in a single push`,
    `限制一次推送中可以更新多少个分支和标签`,
  ],
